...
def extract_tozed_conf(input_file, output_file):
    try:
        ...
        with open(input_file, 'rb') as f:
            f.seek(0x400)
            # Read a reasonable chunk (max 32KB - 0x400)
            raw = f.read(32768 - 0x400)

            # Find the end of the zlib stream
            # Zlib streams end with 0x00 0x00 0xff 0xff (adler32 follows)
            # We'll try to decompress as much as possible
            ...
            # Decompress and output as before
...
def create_mtd7(original_mtd7, modified_config_file, output_file):
    ...
    try:
        ...
        # Read the original MTD7 to get the header and zlib block
        ...
        # Decompress the original zlib block
        try:
        ...
        # If config is unchanged, use the original zlib block for a perfect match
        if config_data == original_config:
            ...
            # Patch header with new check value (MD5 guess)
            import hashlib
            # The check value is usually calculated over the whole partition, but we'll use the new compressed data
            check_value = hashlib.md5(compressed_data).digest()[:16]
            # Patch header at offset 4-19
        ...
        # Create the new MTD7 file
        with open(output_file, 'wb') as f:
            # 1. Write the patched header
            f.write(header)

            # 2. Write the compressed config
            f.write(compressed_data)

            # 3. Pad to 32KB (0x8000) if needed
            if total_size < 32768:
...
if __name__ == "__main__":
    ...
    # Paths for different operations
    ...
    # Check if we're extracting or creating
